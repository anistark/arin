...
| Managing the login agent | `arin service enable\|disable\|status\|restart`, in the binary rather than in `launch-agent.sh`, because the script had to be told where `Arin.app` was and a process can find its own bundle. That argument was the whole bug surface: the documented Homebrew line omitted it and the script fell back to `/Applications/Arin.app`, which is a different build on a machine with both. Not spelled `start`, which would blur the line `-d` draws between running now and running at login. The plist is still generated from `packaging/macos/com.anistark.arin.plist` rather than built in Rust, so the agent has one description. `enable` rewrites a Homebrew keg path to the `opt` alias, since the versioned one dies at the next upgrade, and refuses outright when nix-darwin is managing the same label. |
